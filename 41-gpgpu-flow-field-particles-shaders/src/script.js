...
for (let i = 0; i < baseGeometry.count; i++) {
  ...
  baseParticlesTexture.image.data[i4 + 0] =
    baseGeometry.instance.attributes.position.array[i3 + 0] // x
  ...
  baseParticlesTexture.image.data[i4 + 3] = Math.random() // w (random value)
}
...
for (let y = 0; y < gpgpu.size; y++) {
  for (let x = 0; x < gpgpu.size; x++) {
    ...
    particlesUvArray[i2 + 0] = (x + 0.5) / gpgpu.size // u
    particlesUvArray[i2 + 1] = (y + 0.5) / gpgpu.size // v
